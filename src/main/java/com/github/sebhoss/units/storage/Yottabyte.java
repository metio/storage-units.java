...
public class Yottabyte extends StorageUnit<Yottabyte> {
    ...
    public Yottabyte add(final long bytesToAdd) {
        return new Yottabyte(this.bytes.add(BigInteger.valueOf(bytesToAdd)));
    }

    @Override
    public Yottabyte add(final StorageUnit<?> storageAmount) {
        return new Yottabyte(this.bytes.add(storageAmount.bytes));
    }

    @Override
    public Yottabyte divide(final long divisor) {
        return new Yottabyte(this.bytes.divide(BigInteger.valueOf(divisor)));
    }

    @Override
    public Yottabyte multiply(final long factor) {
        return new Yottabyte(this.bytes.multiply(BigInteger.valueOf(factor)));
    }

    @Override
    public Yottabyte subtract(final long bytesToSubtract) {
        return new Yottabyte(this.bytes.subtract(BigInteger.valueOf(bytesToSubtract)));
    }

    @Override
    public Yottabyte subtract(final StorageUnit<?> storageAmount) {
        return new Yottabyte(this.bytes.subtract(storageAmount.bytes));
    }
