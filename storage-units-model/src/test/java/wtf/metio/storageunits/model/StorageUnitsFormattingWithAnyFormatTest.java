...
class StorageUnitsFormattingWithAnyFormatTest {
    ...
    private static final List<BiFunction<BigInteger, Format, String>> BIG_INTEGER_FORMATTERS = List.of(
    ...
    private static final List<BiFunction<Long, Format, String>> LONG_FORMATTERS = List.of(
            StorageUnits::formatAsBinaryUnit,
            StorageUnits::formatAsKibibyte,
            StorageUnits::formatAsMebibyte,
            StorageUnits::formatAsGibibyte,
    ...
    @TestFactory
    Stream<DynamicTest> shouldFormatBigIntegerAsStorageUnitWithAnyFormat() {
        return BIG_INTEGER_FORMATTERS.stream()
                .flatMap(formatter -> BYTES.stream()
                        .map(bytes -> DynamicTest.dynamicTest(String.format("%s formatted", bytes),
                                () -> Assertions.assertNotNull(formatter.apply(bytes, FORMAT)))));
    }

    @TestFactory
    Stream<DynamicTest> shouldFormatLongAsStorageUnitWithAnyFormat() {
        return LONG_FORMATTERS.stream()
                .flatMap(formatter -> BYTES.stream()
                        .map(bytes -> DynamicTest.dynamicTest(String.format("%s formatted", bytes),
                                () -> Assertions.assertNotNull(formatter.apply(bytes.longValue(), FORMAT)))));
    }
